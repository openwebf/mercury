...
abstract class MercuryThread {
  ...
  /// In dedicated thread mode, Mercury creates a shared buffer to record the UI operations that are generated from the JS thread.
  ...
  /// Setting this value to 0 in dedicated thread mode can achieve 100% concurrency but may reduce isolate speed because the
...
class FlutterUIThread extends MercuryThread {
  FlutterUIThread();
  ...
  @override
  double identity() {
    return (-newIsolateId()).toDouble();
  }
}

/// Executes your JavaScript code in a dedicated thread.
class DedicatedThread extends MercuryThread {
  double? _identity;
  ...
  @override
  double identity() {
    return _identity ?? (newIsolateId()).toDouble();
  }
}
...
class DedicatedThreadGroup {
  int _slaveCount = 0;
  final int _identity = newIsolateId();
