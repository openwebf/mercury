...
// prof: #include "timing/performance.h"
namespace mercury {
...
ExecutingContext::ExecutingContext(DartIsolateContext* dart_isolate_context,
  ...
  if (is_dedicated) {
    ...
    // Bigger size introduce more isolate consistence and lower size led to more high performance by the reason of
    // concurrency.
  ...
  // When the `load` event fired in global, the GC will turn back on.
  ...
  // prof: dart_isolate_context->profiler()->StartTrackSteps("ExecutingContext::InstallBindings");

  // Register all built-in native bindings.
  InstallBindings(this);

  // prof: dart_isolate_context->profiler()->FinishTrackSteps();
  // prof: dart_isolate_context->profiler()->StartTrackSteps("ExecutingContext::InstallGlobal");

  // Binding global object and global.
  InstallGlobal();

  // prof: dart_isolate_context->profiler()->FinishTrackSteps();
  // prof: dart_isolate_context->profiler()->StartTrackSteps("ExecutingContext::InstallPerformance");

  // Install performance
  // prof: InstallPerformance();

  // prof: dart_isolate_context->profiler()->FinishTrackSteps();
  // prof: dart_isolate_context->profiler()->StartTrackSteps("ExecutingContext::initWebFPolyFill");

  initMercuryPolyFill(this);

  // prof: dart_isolate_context->profiler()->FinishTrackSteps();
  // prof: dart_isolate_context->profiler()->StartTrackSteps("ExecutingContext::InitializePlugin");
  ...
  // prof: ui_command_buffer_.AddCommand(UICommand::kFinishRecordingCommand, nullptr, nullptr, nullptr);
}
...
bool ExecutingContext::EvaluateJavaScript(const char* code,
  ...
  // prof: dart_isolate_context_->profiler()->StartTrackSteps("ExecutingContext::EvaluateJavaScript");

  JSValue result;
  if (parsed_bytecodes == nullptr) {
    // prof: dart_isolate_context_->profiler()->StartTrackSteps("JS_Eval");

    result = JS_Eval(script_state_.ctx(), code, code_len, sourceURL, JS_EVAL_TYPE_GLOBAL);

    // prof: dart_isolate_context_->profiler()->FinishTrackSteps();
  } else {
    // prof: dart_isolate_context_->profiler()->StartTrackSteps("JS_Eval");
    ...
    *bytecode_len = len;
    ...
    // prof: dart_isolate_context_->profiler()->FinishTrackSteps();
    // prof: dart_isolate_context_->profiler()->StartTrackSteps("JS_EvalFunction");
...
bool ExecutingContext::EvaluateByteCode(uint8_t* bytes, size_t byteLength) {
  // prof: dart_isolate_context_->profiler()->StartTrackSteps("ExecutingContext::EvaluateByteCode");
...
void ExecutingContext::DrainMicrotasks() {
  // prof: dart_isolate_context_->profiler()->StartTrackSteps("ExecutingContext::DrainMicrotasks");

  DrainPendingPromiseJobs();

  // prof: dart_isolate_context_->profiler()->FinishTrackSteps();

  // prof ui_command_buffer_.AddCommand(IsolateCommand::kFinishRecordingCommand, nullptr, nullptr, nullptr);
}
...
void ExecutingContext::DrainPendingPromiseJobs() {
  ...
  // prof: dart_isolate_context_->profiler()->StartTrackSteps("JS_ExecutePendingJob");
...
uint8_t* ExecutingContext::DumpByteCode(const char* code,
  ...
  // prof: dart_isolate_context_->profiler()->StartTrackSteps("JS_WriteObject");
...
void ExecutingContext::FlushIsolateCommand(const BindingObject* self, uint32_t reason) {
  std::vector<NativeBindingObject*> deps;
  FlushIsolateCommand(self, reason, deps);
}

void ExecutingContext::FlushIsolateCommand(const webf::BindingObject* self,
                                      uint32_t reason,
                                      std::vector<NativeBindingObject*>& deps) {
  if (!isolateCommandBuffer()->empty()) {
    if (is_dedicated_) {
      bool should_swap_isolate_commands = false;
      if (isIsolateCommandReasonDependsOnObject(reason)) {
        bool object_mounted_on_dart = self->bindingObject()->invoke_bindings_methods_from_native != nullptr;
        bool is_deps_objects_mounted_on_dart = true;

        for (auto binding : deps) {
          if (binding->invoke_bindings_methods_from_native == nullptr) {
            is_deps_objects_mounted_on_dart = false;
          }
        }

        if (!object_mounted_on_dart || !is_deps_objects_mounted_on_dart) {
          should_swap_isolate_commands = true;
        }
      }

      if (isIsolateCommandReasonDependsOnLayout(reason) || isIsolateCommandReasonDependsOnAll(reason)) {
        should_swap_isolate_commands = true;
      }

      // Sync commands to dart when caller dependents on Object.
      if (should_swap_isolate_commands) {
        isolate_command_buffer_.SyncToActive();
      }
    }

    dartMethodPtr()->flushIsolateCommand(is_dedicated_, context_id_, self->bindingObject());
  }
}
