...
NativeValue* handleInvokeModuleTransientCallback(void* ptr,
  ...
  // prof: context->dartIsolateContext()->profiler()->StartTrackSteps("handleInvokeModuleTransientCallback");
...
ScriptValue ModuleManager::__mercury_invoke_module__(ExecutingContext* context,
  ...
  // prof: context->dartIsolateContext()->profiler()->StartTrackLinkSteps("Call To Dart");
  ...
  if (callback != nullptr) {
    ...
    result = context->dartMethodPtr()->invokeModule(context->isDedicated(), module_context.get(), context->contextId(),
                                                    // prof: context->dartIsolateContext()->profiler()->link_id(),
  ...
  // prof: context->dartIsolateContext()->profiler()->FinishTrackLinkSteps();
