...
void Dispatcher::Dispose(mercury::multi_threading::Callback callback) {
#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: BEGIN EXE OPAQUE FINALIZER ";
#endif

  for (auto&& thread : js_threads_) {
    auto* isolate_group = static_cast<IsolateGroup*>(thread.second->opaque());
    for (auto& isolate : (*isolate_group->isolates())) {
...
#if ENABLE_LOG
    MERCURY_LOG(VERBOSE) << "[Dispatcher]: BEGIN EXEC SYNC DART WORKER";
#endif
    dart_work(true);
#if ENABLE_LOG
    MERCURY_LOG(VERBOSE) << "[Dispatcher]: FINISH EXEC SYNC DART WORKER";
#endif
  }

#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: BEGIN FINALIZE ALL JS THREAD";
#endif
...
#if ENABLE_LOG
  if (is_sync) {
    MERCURY_LOG(WARN) << " SYNC BLOCK THREAD " << std::this_thread::get_id() << " FOR A DART CALLBACK TO RECOVER";
  }
#endif
...
void Dispatcher::FinalizeAllJSThreads(mercury::multi_threading::Callback callback) {
  ...
  for (auto&& thread : js_threads_) {
    PostToJs(
...
#if ENABLE_LOG
          MERCURY_LOG(VERBOSE) << "[Dispatcher]: RUN JS FINALIZER, context_id: " << thread.first;
#endif
          looper->ExecuteOpaqueFinalizer();
          unfinished_thread--;

#if ENABLE_LOG
          MERCURY_LOG(VERBOSE) << "[Dispatcher]: UNFINISHED THREAD: " << unfinished_thread;
#endif
          if (unfinished_thread == 0) {
            is_final_async_dart_task_complete = true;
            return;
        ...
        thread.second.get());
#if ENABLE_LOG
    MERCURY_LOG(VERBOSE) << "[Dispatcher]: POST TO JS THREAD";
#endif
  }

#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: WAITING FOR JS THREAD COMPLETE";
#endif
...
#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: ALL JS THREAD FINALIZED SUCCESS";
#endif
  callback();
}
...
#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: FINISH EXEC OPAQUE FINALIZER ";
#endif
  for (auto&& thread : js_threads_) {
    thread.second->Stop();
  }
#if ENABLE_LOG
  MERCURY_LOG(VERBOSE) << "[Dispatcher]: ALL THREAD STOPPED";
#endif
}
